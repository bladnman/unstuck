...
- 2026-03-23: The local always-on dashboard service is managed by `node scripts/manage_dashboard_service.mjs`. On macOS it installs LaunchAgent `com.unstuck.dashboard`, keeps the app on `127.0.0.1:4004`, uses `http://unstuck.localhost:4004` as the readable browser URL, and creates `~/Applications/Unstuck.webloc` as the easiest launch point.
